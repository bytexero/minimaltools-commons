...
import com.minimaltools.commons.models.SharedTheme
import com.minimaltools.commons.views.*
...
fun Context.updateTextColors(viewGroup: ViewGroup) {
    ...
    (0 until cnt).map { viewGroup.getChildAt(it) }.forEach {
        when (it) {
            is CustomTextView -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomAppCompatSpinner -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomCompatRadioButton -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomAppCompatCheckbox -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomEditText -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomAutoCompleteTextView -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomFloatingActionButton -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomSeekBar -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomButton -> it.setColors(textColor, accentColor, backgroundColor)
            is CustomTextInputLayout -> it.setColors(textColor, accentColor, backgroundColor)
